package com.vcoders.on_demand_youtube_player.utils;

public class Constant {
    public static String VIDEO_YOUTUBE_ID = "videoYoutubeId";
    public static String PLAYLIST_ID = "playlistId";
    public static String DESCRIPTION = "description";
    public static String TOPICS = "topics";
    public static String VIDEOS = "videos";
    public static String SEARCH_NAME = "name";
    public static String POSITION = "position";
    public static String BASE_URL_YOUTUBE = "https://www.googleapis.com/youtube/v3/";
    public static String BASE_URL_ACCOUNT = "https://www.googleapis.com/oauth2/v3/";
    public static String API_KEY = "AIzaSyBENTkoHFtO8tDHGC5Mcuzya9kT00kIusg";
    public static String ACCOUNT_UTILS = "accountUtils";
    public static String PREFERENCE_NAME = "onDemandYoutubePlayer";
}
