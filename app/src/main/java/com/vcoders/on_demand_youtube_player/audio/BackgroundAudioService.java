...
import com.vcoders.on_demand_youtube_player.audio.youtubeExtractor.VideoMeta;
import com.vcoders.on_demand_youtube_player.audio.youtubeExtractor.YouTubeExtractor;
import com.vcoders.on_demand_youtube_player.audio.youtubeExtractor.YouTubeUriExtractor;
...
public class BackgroundAudioService extends Service implements AudioManager.OnAudioFocusChangeListener, MediaPlayer.OnCompletionListener,
    ...
    private volatile boolean audioNoisyReceiverRegistered, mediaButtonReceiverRegistered;
    ...
    private ListIterator<Video> iterator;
    ...
    private final IntentFilter mediaButtonFilter = new IntentFilter(Intent.ACTION_MEDIA_BUTTON);
...
//        mediaPlayer.setWakeMode(getApplicationContext(), PowerManager.PARTIAL_WAKE_LOCK);
    ...
    private void handleMedia(Intent intent) {
        ...
        switch (intentMediaType) {
            ...
            case Config.YOUTUBE_MEDIA_TYPE_VIDEO:
                ...
                currentVideo = (Video) intent.getParcelableExtra(Config.KEY_YOUTUBE_TYPE_VIDEO);
            ...
            case Config.YOUTUBE_MEDIA_TYPE_PLAYLIST:
                ...
                youTubeVideos = (ArrayList<Video>) intent.getParcelableExtra(Config.KEY_YOUTUBE_TYPE_PLAYLIST);
    ...
    private void buildNotification(NotificationCompat.Action action) {
        ...
        notificationBuilder.setContentTitle(currentVideo.getSnippet() != null ? currentVideo.getSnippet().getTitle() : "");
        notificationBuilder.setContentInfo(currentVideo.getSnippet() != null ? currentVideo.getSnippet().getDuration() : "");
        notificationBuilder.setUsesChronometer(true);
        notificationBuilder.setContentIntent(clickPendingIntent);
        notificationBuilder.setDeleteIntent(stopPendingIntent);
        notificationBuilder.setSubText(Utils.getInstance().formatViewCount(currentVideo.getContentDetails() == null ? null :
                Utils.getInstance().convertIntToString(currentVideo.getContentDetails().getItemCount())));
        notificationBuilder.setStyle(style);

        //load bitmap for largeScreen
        if (currentVideo.getSnippet() != null && currentVideo.getSnippet().getThumbnails() != null &&
                currentVideo.getSnippet().getThumbnails().getMedium() != null && currentVideo.getSnippet().getThumbnails().getMedium().getUrl() != null) {
            Picasso.with(this)
                    .load(currentVideo.getSnippet().getThumbnails().getMedium().getUrl())
...
//        }
    ...
    private void registerMediaButtonReceiver() {
        if (!mediaButtonReceiverRegistered) {
